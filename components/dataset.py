...
class Batch(object):
    ...
    def src_sents_var(self):
        return nn_utils.to_input_variable(self.src_sents, self.vocab.source,
                                          cuda=self.cuda)
    def src_sents_vars(self,bert_model):
        return nn_utils.to_input_variable1(self.src_sents, self.vocab.source,bert_model,
                                          cuda=self.cuda)
    def src_sents_varss(self,bert_model):
        return nn_utils.to_input_variable2(self.src_sents,[e.table for e in self.examples] ,self.tokenizer,bert_model,
                                          cuda=self.cuda)    
    @cached_property
