...
class VocabEntry(object):
    ...
    def copyandmerge(self,tokenizer):
        tmpvocab=copy.deepcopy(tokenizer.vocab)
        tmpvocab=collections.OrderedDict(tmpvocab)
        for item in self.word2id:
            if item not in tmpvocab:
                tmpvocab[item]=len(tmpvocab)
        self.word2id=tmpvocab
