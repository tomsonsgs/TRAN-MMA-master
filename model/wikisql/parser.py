...
from dependency import constructbatch,constructbatchforextra

@Registrable.register('wikisql_parser')
class WikiSqlParser(Parser):
    def __init__(self, args, vocab, transition_system,tmpvocab,tmpvocab1,tmpvocab2,tmpvocab3):
        super(WikiSqlParser, self).__init__(args, vocab, transition_system,tmpvocab,tmpvocab1,tmpvocab2,tmpvocab3)

        self.table_header_lstm = nn.LSTM(768, int(args.hidden_size / 2), bidirectional=True, batch_first=True)
    ...
    def encode_table_header1(self, tables,src_head,table_col_lens):
...
#        table_head_word_embeds = self.src_embed(table_head_wids.view(-1)).view(list(table_head_wids.size()) + [self.src_embed.embedding_dim])
        table_head_word_embeds=src_head
        ...
        return column_word_encodings, table_header_encoding, table_header_mask,table_head_word_embeds
    ...
    def score(self, examples, return_encode_state=False,weights=None):
        """
...
#        src_encodings,table_header_encoding,length=batch.src_sents_varss(self.bert_model) 
#        mains,adj1,adj2,edge1,edge2,mask1=constructbatch(examples)
        cs,ps,ns,ts,tins,f1=constructbatchforextra(examples)
        ps=torch.cat([self.emb11(ps),cs.unsqueeze(-1)],-1)
        ns=torch.cat([self.emb22(ns),ps],-1)
        ts=self.emb33(ts)
        tins=self.emb44(tins)
        src_encodings,table_header_encoding,length=batch.src_sents_varss(self.bert_model)
        src_encodings, (last_state, last_cell) = self.encode2(torch.cat([ns,src_encodings],-1).transpose(0,1), batch.src_sents_len)
#        print(batch.src_sents_vars(self.bert_model).size())
#        print(src_encodings.size(1))
#        print(adj1.size(1))
#        a=input('hasha')
#        src_encodings,graph_pre=self.gcn(src_encodings,mains,adj1,adj2,edge1,edge2)
        graph_pre=last_cell
#        print(src_encodings.size())
#        src_encodings=self.cross(src_encodings,src_encodings,batch.src_token_mask)
#        src_encodings, (last_state, last_cell) = self.encode1(src_encodings.transpose(0,1), batch.src_sents_len)
        dec_init_vec = self.init_decoder_state(last_state, last_cell,graph_pre)
        
#        src_token_embed = self.src_embed(batch.src_sents_var).transpose(0,1)
        column_word_encodings, table_header_encoding, table_header_mask,table_head_word_embeds = self.encode_table_header1([e.table for e in examples],table_header_encoding,length)
#        mul_atts=self.cross(src_token_embed,table_head_word_embeds,table_header_mask)
        table_header_encoding=torch.tanh(self.linear88(torch.cat([table_header_encoding,ts,tins],-1)))
        mul_att=self.cross1(src_encodings,table_header_encoding,table_header_mask)
        mul_atts=f1
        mul_att=mul_att+2*mul_atts
        mul_att=mul_att/torch.sum(mul_att,-1).unsqueeze(-1)
        h_tm1 = dec_init_vec
        h_ex=torch.zeros(src_encodings.size(0),args.hidden_size).float().cuda()
        global_state=torch.zeros(src_encodings.size(0),args.hidden_size).float().cuda()
        h_ex=torch.tanh(self.decoder_cell_init(torch.cat([last_cell,graph_pre],-1)))
...
#        h_0 = F.tanh(h_0)
        ...
        word_prob=[torch.zeros(src_encodings.size(1)).float().cuda() for example in examples]
        word_prob_loss=[torch.zeros(src_encodings.size(1)).float().cuda() for example in examples]
        select_probs = [torch.zeros(table_header_encoding.size(1)).float().cuda() for example in examples]
        def process(examples,num):
            tmp=torch.zeros(table_header_encoding.size(0),table_header_encoding.size(1)).float().cuda()
            for idx,exa in enumerate(examples):
                for act in exa.tgt_actions:
                    if isinstance(act.action,WikiSqlSelectColumnAction):
                        tmp[idx,act.action.column_id]+=1.0
            return tmp
        gold_select=process(examples,table_header_encoding.size(1))
        last_used=torch.zeros(src_encodings.size(0),src_encodings.size(1)).float().cuda()
        for t in range(batch.max_action_num):
            ...
            if t == 0:
                ...
                x_ex=Variable(self.new_tensor(len(batch), args.action_embed_size+args.hidden_size).zero_(), requires_grad=False)
                x_ex=[Variable(self.new_tensor(len(batch), args.action_embed_size).zero_(), requires_grad=False),Variable(self.new_tensor(len(batch), args.hidden_size).zero_(), requires_grad=False)]
                ...
                if args.no_parent_state is False:
                    parent_states = torch.stack([history_states[p_t][0][batch_id][-args.hidden_size:]
                                                 for batch_id, p_t in
                                                 enumerate(a_t.parent_t if a_t else 0 for a_t in actions_t)])

                    parent_cells = torch.stack([history_states[p_t][1][batch_id][-args.hidden_size:]
                                                for batch_id, p_t in
                                                enumerate(a_t.parent_t if a_t else 0 for a_t in actions_t)])
                ...
                if args.no_parent_state_ex is False:
                    parent_states = torch.stack([history_states[p_t][0][batch_id][-args.hidden_size:]
                                                 for batch_id, p_t in
                                                 enumerate(a_t.parent_t if a_t else 0 for a_t in actions_t)])
...
#                                                enumerate(a_t.parent_t if a_t else 0 for a_t in actions_t)])
                    ...
                    if args.lstm == 'parent_feed':
                        ...
                        inputs_ex.append(parent_states)        

                x = torch.cat(inputs, dim=-1)
#                x_ex = torch.cat(inputs_ex, dim=-1)
                x_ex=inputs_ex
            (h_t, cell_t),h_ex, att_t,alpha_t, last_used ,global_state,_= self.step(x, h_tm1,x_ex,h_ex,global_state, src_encodings,
                                             utterance_encodings_att_linear,last_used,torch.stack(word_prob,0),
            ...
            column_attention_weights=torch.sum(alpha_t.unsqueeze(-1)*mul_att,-2)
            action_prob = F.softmax(self.action_gate2(torch.tanh(self.action_gate1(att_t))), dim=-1)
            ...
            for e_id, example in enumerate(examples):
                if t < len(example.tgt_actions):
                    ...
                    if isinstance(action_t, ApplyRuleAction):
                        ...
                        select_probs[e_id]=select_probs[e_id]+column_attention_weights[e_id]
                        tmp,_=torch.min(torch.stack([word_prob[e_id],alpha_t[e_id]],-1),-1)
                        word_prob_loss[e_id]=word_prob_loss[e_id]+tmp
                        word_prob[e_id]=word_prob[e_id]+alpha_t[e_id]
                    elif isinstance(action_t, GenTokenAction):
                        tmp,_=torch.min(torch.stack([word_prob[e_id],primitive_predictor_prob[e_id, 1] *alpha_t[e_id]],-1),-1)
                        word_prob_loss[e_id]=word_prob_loss[e_id]+tmp
                        word_prob[e_id]=word_prob[e_id]+primitive_predictor_prob[e_id, 1] * alpha_t[e_id]
                    ...
                    if weights is None:
#                        if(t==1):
#                            action_probs[e_id].append(10*((act_prob_t_i+0.0000000001).log()))
#                        else:
                          action_probs[e_id].append((act_prob_t_i+0.0000000001).log())
                    else:
                        action_probs[e_id].append((act_prob_t_i+0.0000000001).log()*weights[e_id][t])
#                     action_probs[e_id].append(act_prob_t_i.log())

            history_states.append((h_t, cell_t))
            att_vecs.append(att_t)
            history_states_ex.append(h_ex)
            h_tm1 = (h_t, cell_t)
            att_tm1 = att_t
#        print()
#        print([action_probs_i[0] for action_probs_i in action_probs])
        # sum all the action probabilities
        action_prob_var = torch.stack([torch.stack(action_probs_i).sum() for action_probs_i in action_probs])
        select_var=(torch.stack(select_probs,0)-gold_select)*(torch.stack(select_probs,0)-gold_select)
        select_var=torch.mean(select_var)
        loss_var=torch.sum(torch.stack(word_prob_loss,0),-1)
#        action_prob_var = torch.stack([torch.stack(action_probs_i).log().sum() for action_probs_i in action_probs])
        return [action_prob_var,select_var,loss_var],action_probs   # TODO: supervised attention not implemented yet!

    def parse(self, question, context, beam_size=5,or_example=None,doubletest=True):
        table = context
        args = self.args
        examples=[or_example]
#        mains,adj1,adj2,edge1,edge2,mask1=constructbatch(examples)
        ...
        ts=self.emb33(ts)
        tins=self.emb44(tins)
        src_sent_var,table_header_encoding,length = nn_utils.to_input_variable2([question], [table],self.tokenizer,self.bert_model,
                                                  cuda=self.args.cuda, training=False)
#        src_encodings,table_header_encoding,length=batch.src_sents_varss(self.bert_model)
        utterance_encodings, (last_state, last_cell) = self.encode2(torch.cat([ns,src_sent_var],-1).transpose(0,1), [len(question)])
#        utterance_encodings,graph_pre=self.gcn(utterance_encodings,mains,adj1,adj2,edge1,edge2)
        graph_pre=last_cell
#        utterance_encodings, (last_state, last_cell) = self.encode(src_sent_var, [len(question)])
#        utterance_encodings=self.cross(utterance_encodings,utterance_encodings)
#        utterance_encodings, (last_state, last_cell) = self.encode1(utterance_encodings.transpose(0,1), [len(question)])
        dec_init_vec = self.init_decoder_state(last_state, last_cell,graph_pre)
#        src_token_embed = self.src_embed(nn_utils.to_input_variable([question], self.vocab.source,cuda=self.args.cuda, training=False))
        column_word_encodings, table_header_encoding, table_header_mask,table_head_word_embeds = self.encode_table_header1([table],table_header_encoding,length)
#        mul_atts=self.cross(src_token_embed.transpose(0,1),table_head_word_embeds,table_header_mask)
        table_header_encoding=torch.tanh(self.linear88(torch.cat([table_header_encoding,ts,tins],-1)))
        mul_att=self.cross1(utterance_encodings,table_header_encoding,table_header_mask)
        if(doubletest):
         mul_atts=f1   
         mul_att=mul_att+2*mul_atts
         mul_att=mul_att/torch.sum(mul_att,-1).unsqueeze(-1)
        h_tm1 = dec_init_vec
        h_ex=torch.zeros(utterance_encodings.size(0),args.hidden_size).float().cuda()
        global_state=torch.zeros(utterance_encodings.size(0),args.hidden_size).float().cuda()
#        h_ex=torch.tanh(self.decoder_cell_init(last_state))
        h_ex=torch.tanh(self.decoder_cell_init(torch.cat([last_cell,graph_pre],-1)))
        # (batch_size, query_len, hidden_size)
...
#        print
        ...
        completed_atts=[]
        completed_attss=[]
        last_used=torch.zeros(utterance_encodings.size(0),utterance_encodings.size(1)).float().cuda()
        word_prob=torch.zeros(utterance_encodings.size(0),utterance_encodings.size(1)).float().cuda()
        while len(completed_hypotheses) < beam_size and t < self.args.decode_max_time_step:
            ...
            if t == 0:
                ...
                x_ex=Variable(self.new_tensor(1, args.action_embed_size+args.hidden_size).zero_(), requires_grad=False)
                x_ex=[Variable(self.new_tensor(1, args.action_embed_size).zero_(), requires_grad=False),Variable(self.new_tensor(1, args.hidden_size).zero_(), requires_grad=False)]
                if args.no_parent_field_type_embed is False:
                    offset = args.action_embed_size  # prev_action
                    offset += args.hidden_size * (not args.no_input_feed)
                    offset += args.action_embed_size * (not args.no_parent_production_embed)
                ...
                if args.no_parent_state is False:
                    ...
                    parent_states = torch.stack([hyp_states[hyp_id][p_t][0][-args.hidden_size:] for hyp_id, p_t in enumerate(p_ts)])
                    parent_cells = torch.stack([hyp_states[hyp_id][p_t][1][-args.hidden_size:] for hyp_id, p_t in enumerate(p_ts)])
...
#                                             src_token_mask=None)
            (h_t, cell_t), h_ex,att_t,alpha_t, last_used,global_state,atts = self.step(x, h_tm1, x_ex,h_ex,global_state,exp_src_encodings,
                                             exp_src_encodings_att_linear,last_used,word_prob,
                                             src_token_mask=None)
            # ApplyRule action probability
            # (batch_size, grammar_size)
            action_prob = F.softmax(self.action_gate2(torch.tanh(self.action_gate1(att_t))), dim=-1)
            apply_rule_log_prob = torch.log(torch.softmax(self.production_readout(att_t), dim=-1)+0.000000001)
            ...
            column_attention_weights = self.column_pointer_net(table_header_encoding, table_header_mask,
                                                               att_t.unsqueeze(0)).squeeze(0)
            column_attention_weights=torch.sum(alpha_t.unsqueeze(-1)*mul_att,-2)
            column_selection_log_prob = torch.log(column_attention_weights+0.000000001)
...
#                                                       att_t.unsqueeze(0)).squeeze(0)
                    ...
                    if action_type == ApplyRuleAction:
                        ...
                        for col_id, column in enumerate(table.header):
                            ...
                            meta_entry = {'action_type': 'sel_col', 'col_id': col_id,
                                          ...
                                          'prev_hyp_id': hyp_id,'tmps':tmps}
                            new_hyp_meta.append(meta_entry)
                    elif action_type == GenTokenAction:
                        tmps=primitive_predictor_prob[hyp_id, 1]*alpha_t[hyp_id]
                        ...
                        for token_pos in valid_token_pos_list:
                            ...
                            score_copy = torch.log(p_copy+0.000000001)
                        ...
                        if hyp._value_buffer:
                            ...
                            eos_score = torch.log(eos_prob+0.000000001)
            ...
            for new_hyp_score, meta_id in zip(top_new_hyp_scores.data.cpu(), meta_ids.data.cpu()):
                ...
                if new_hyp.completed:
                    ...
                    completed_atts.append(att_history[prev_hyp_id] + [atts[prev_hyp_id]])
                    completed_attss.append(att_historys[prev_hyp_id] + [mul_att[0].cpu().data.numpy()])
                else:
                    new_hypotheses.append(new_hyp)
                    live_hyp_ids.append(prev_hyp_id)
                    live_hyp_tmps.append(hyp_meta_entry['tmps'])

            if live_hyp_ids:
                hyp_states = [hyp_states[i] + [(h_t[i], cell_t[i])] for i in live_hyp_ids]
                hyp_states_ex=[hyp_states_ex[i] + [h_ex[i]] for i in live_hyp_ids]
                att_history=[att_history[i] + [atts[i]] for i in live_hyp_ids]
                att_historys=[att_historys[i] + [mul_att[0].cpu().data.numpy()] for i in live_hyp_ids]
                h_tm1 = (h_t[live_hyp_ids], cell_t[live_hyp_ids])
                h_ex=h_ex[live_hyp_ids]
                global_state=global_state[live_hyp_ids]
                last_used = last_used[live_hyp_ids]
                word_prob=word_prob[live_hyp_ids]
                for idx,item in enumerate(live_hyp_tmps):
                    if item is not None:
                        word_prob[idx]=word_prob[idx]+item
                att_tm1 = att_t[live_hyp_ids]
                hypotheses = new_hypotheses
                t += 1
            else: break
        completed_atts=list(enumerate(completed_atts))
        completed_atts.sort(key=lambda x: -completed_hypotheses[x[0]].score)
        completed_atts=[item[1] for item in completed_atts]
        completed_attss=list(enumerate(completed_attss))
        completed_attss.sort(key=lambda x: -completed_hypotheses[x[0]].score)
        completed_attss=[item[1] for item in completed_attss]
        completed_hypotheses.sort(key=lambda hyp: -hyp.score)

        return completed_hypotheses,(completed_atts, completed_attss)
    def sample(self, question, context, beam_size=1,show=False):
        table = context
        args = self.args
        ...
        while len(completed_hypotheses) < beam_size and t < self.args.decode_max_time_step:
            ...
            new_hyp_scores = torch.stack([x['new_hyp_score'].cuda() for x in new_hyp_meta])
            new_scores = torch.stack([torch.exp(x['score'].cuda()) for x in new_hyp_meta])
            if(show):
             print(new_hyp_scores)
             print(new_scores)
            ids=torch.multinomial(new_scores+0.00000001, 1)
            meta_ids=ids
            top_new_hyp_scores=new_hyp_scores[ids]
            extra.append(new_scores[ids])
#            top_new_hyp_scores, meta_ids = torch.topk(new_hyp_scores,
#                                                      k=min(new_hyp_scores.size(0),
#                                                            beam_size - len(completed_hypotheses)))
#            print(top_new_hyp_scores)
            live_hyp_ids = []
            new_hypotheses = []
            try:
                for new_hyp_score, meta_id in zip(top_new_hyp_scores.data.cpu(), meta_ids.data.cpu()):
        ...
        return completed_hypotheses,extra