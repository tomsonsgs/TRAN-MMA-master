...
from model.nn_utils import LabelSmoothing,DMN,cross_atts,multi_rnn,multi_rnn_withcontext,ParentGRU,cross_attss,myGCN
...
class Parser(nn.Module):
    ...
    def __init__(self, args, vocab, transition_system,tmpvocab,tmpvocab1,tmpvocab2,tmpvocab3):
        ...
        args.decoder_num=1
        args.no_parent_state_ex=False
        self.args = args
        self.vocab = vocab
        self.tmpvocab=tmpvocab
        self.emb11=torch.nn.Embedding(len(tmpvocab1),args.embed_size)
        self.emb22=torch.nn.Embedding(len(tmpvocab2),args.embed_size)
        self.emb33=torch.nn.Embedding(len(tmpvocab3),args.embed_size)
        self.emb44=torch.nn.Embedding(3,args.embed_size)
        self.linear88=nn.Linear(args.embed_size*2+args.hidden_size, args.hidden_size)
        self.transition_system = transition_system
        self.grammar = self.transition_system.grammar
#        self.gcn=myGCN(args.hidden_size,256,len(tmpvocab))
        ...
        self.dmn=DMN(args.hidden_size,args.hidden_size)
        self.cross=cross_atts(args.hidden_size,args.hidden_size)
        self.cross1=cross_attss(args.hidden_size,args.hidden_size)
        # LSTMs
        if args.lstm == 'lstm':
            self.encoder_lstm = nn.LSTM(args.embed_size, int(args.hidden_size / 2), bidirectional=True)
            self.encoder_lstm1 = nn.LSTM(args.hidden_size, int(args.hidden_size / 2), bidirectional=True)
            self.encoder_lstm2 = nn.LSTM(768+2*args.embed_size+1, int(args.hidden_size / 2), bidirectional=True)
...
#            self.rnns=multi_rnn(input_dim, args.hidden_size,args.decoder_num)
#            self.rnns=multi_rnn_withcontext(input_dim+args.hidden_size, args.hidden_size,args.decoder_num)
#            self.rnns=multi_rnn_withcontext(args.hidden_size, args.hidden_size,args.decoder_num)
            self.rnns=multi_rnn_withcontext(input_dim, args.hidden_size,args.decoder_num)
#            self.tree_encoder=torch.nn.GRUCell(args.action_embed_size,args.hidden_size)
            self.tree_encoder=torch.nn.GRUCell(input_dim,args.hidden_size)
#            self.tree_encoder=ParentGRU(input_dim,args.hidden_size)
        ...
        if args.no_copy is False:
            ...
            self.action_gate1 = nn.Linear(args.att_vec_size, 128)
            self.action_gate2 = nn.Linear(128, 3)
        ...
        self.decoder_cell_init = nn.Linear(2*args.hidden_size, args.hidden_size)
        self.decoder_cell_init1 = nn.Linear(args.hidden_size, args.hidden_size)
        # attention: dot product attention
        # project source encoding to decoder rnn's hidden space
        self.linear1 = nn.Linear(args.hidden_size, args.att_vec_size)
        self.linear2 = nn.Linear(args.hidden_size+1, args.hidden_size, bias=False)
    ...
    def encode1(self, src_token_embed, src_sents_len):
        """Encode the input natural language utterance
...
#        src_token_embed = self.src_embed(src_sents_var)
        ...
        # src_encodings: (tgt_query_len, batch_size, hidden_size)
        src_encodings, (last_state, last_cell) = self.encoder_lstm1(packed_src_token_embed)
        src_encodings, _ = pad_packed_sequence(src_encodings)
        # src_encodings: (batch_size, tgt_query_len, hidden_size)
        src_encodings = src_encodings.permute(1, 0, 2)
        ...
        return src_encodings, (last_state, last_cell)
    def encode2(self, src_token_embed, src_sents_len):
        """Encode the input natural language utterance
...
#        src_token_embed = self.src_embed(src_sents_var)
        ...
        return src_encodings, (last_state, last_cell)
    def init_decoder_state(self, enc_last_state, enc_last_cell,extra_state):
        """Compute the initial decoder hidden state and cell state"""

        h_0 = self.decoder_cell_init(torch.cat([enc_last_cell,extra_state],-1))
        h_0 = F.tanh(h_0)

        return torch.cat([h_0]*self.args.decoder_num,-1), torch.cat([Variable(self.new_tensor(h_0.size()).zero_())]*self.args.decoder_num,-1)
    ...
    def step(self, x, h_tm1,x_ex, h_tm1_ex,global_state,src_encodings, src_encodings_att_linear, last_used,probs,src_token_mask=None, return_att_weight=False):
...
#        print(x_ex[1].size())
#        print(h_tm1_ex.size())
#        h_ex=self.tree_encoder(x_ex[0],h_tm1_ex,x_ex[1])
#        h_ex=self.tree_encoder(x,h_tm1_ex,x_ex[1])
        h_ex=self.tree_encoder(x,h_tm1_ex)
        # h_t: (batch_size, hidden_size)
#        h_t, cell_t = self.decoder_lstm(x, h_tm1)
#        h_t, cell_t = self.rnns(x, h_tm1[0],h_tm1[1])
#        x=torch.cat([x,h_ex],-1)
#        x=h_ex
        h_t, cell_t ,ctx_t,alpha_t,global_state,atts= self.rnns(x, h_tm1[0],h_tm1[1],global_state,src_encodings, src_encodings_att_linear,probs,masks=src_token_mask)
#        alpha_t,ctx_t, last_used=self.dmn(h_t,src_encodings, src_token_mask,last_used)
#        src_encodings_att_linear=self.linear2(torch.cat([src_encodings,last_used.unsqueeze(-1)],-1))
#        ctx_t, alpha_t = nn_utils.dot_prod_attention(h_t[:,-self.args.hidden_size:],
#                                                     src_encodings, src_encodings_att_linear,
#                                                     mask=src_token_mask)

        att_t = F.tanh(self.att_vec_linear(torch.cat([h_t[:,-self.args.hidden_size:], ctx_t], 1)))  # E.q. (5)
#        att_t = self.dropout(torch.tanh(self.linear1(h_0)))
        att_t = self.dropout(att_t)
        if return_att_weight:
            return (h_t, cell_t), h_ex,att_t, alpha_t, last_used,global_state,atts
        else: return (h_t, cell_t), h_ex,att_t, alpha_t, last_used,global_state,atts
        ...
        for t in range(batch.max_action_num):
            ...
            if t == 0:
                ...
                if args.no_parent_state_ex is False:
                    parent_states = torch.stack([history_states_ex[p_t][0][batch_id]
                                                 for batch_id, p_t in
                                                 enumerate(a_t.parent_t if a_t else 0 for a_t in actions_t)])
...
#                                                enumerate(a_t.parent_t if a_t else 0 for a_t in actions_t)])
    ...
    @classmethod
    def load(cls, model_path, cuda=False,bert=None,tmpv=None,v1=None,v2=None,v3=None):
#        bertmodels=BertModel.from_pretrained('./pretrained_models/base-uncased/')
        ...
        parser = cls(saved_args, vocab, transition_system,tmpv,v1,v2,v3)
#        bert.resize_token_embeddings(len(vocab.source))
        parser.bert_model=bert
