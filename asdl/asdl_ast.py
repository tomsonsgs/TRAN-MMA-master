...
class AbstractSyntaxTree(object):
    ...
    def sort_removedup_self(self):
        for field in self.fields:
            if field.cardinality == 'multiple':
                if isinstance(field.type,ASDLCompositeType):
#                    print(field.value)
                    tmp=[value.sort_removedup_self() for value in field.value]
                    tmp=sorted(tmp,key=lambda x:x.to_string())
                    tmps=[]
                    field.value=[]
                    for item in tmp:
                        if(item.to_string() not in tmps):
                           tmps.append(item.to_string())
                           field.value.append(item)
#                    field.value=[value.sort_self() for value in tmp]
                else:field.value=sorted(field.value,key=lambda x:x)
        return self
